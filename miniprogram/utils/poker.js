/* 扑克通用逻辑(与 H5 版 index.html 保持一致) */
const SUITS = ['s', 'h', 'd', 'c'];
const SUIT_CH = { s: '♠', h: '♥', d: '♦', c: '♣' };

function rankStr(r) { return r === 14 ? 'A' : r === 13 ? 'K' : r === 12 ? 'Q' : r === 11 ? 'J' : String(r); }
function cup(x) { return x % 1 === 0 ? String(x) : x.toFixed(1); }
function disp(c) { return { rs: rankStr(c.r), sc: SUIT_CH[c.s], red: (c.s === 'h' || c.s === 'd') }; }
...
module.exports = {
  SUITS, SUIT_CH, rankStr, cup, disp, freshDeck,
  eval5, cmp, combos, eval7, handName, bestHandName, bjHandVal, bjIsBJ
};
